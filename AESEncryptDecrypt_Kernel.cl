...
void
AES_128_decrypt(__global uchar *in,
		__global uchar *out,
		uint *rk,
		__local uint Td0[],
		__local uint Td1[],
...
__kernel
void AES_cbc_128_decrypt(__global uchar *in_all,
					  ...
					  )
{
	...
	__local uint shared_Td1[256];
	__local uint shared_Td2[256];
	__local uint shared_Td3[256];
	__local uchar shared_Td4[256];
	__local uint shared_Rcon[10];
	__local uint shared_Te0[256];
	__local uint shared_Te1[256];
	__local uint shared_Te2[256];
	...
	/* Locate data */
	__global uchar *in = idx * AES_BLOCK_SIZE + in_all;
	__global uchar *out = idx * AES_BLOCK_SIZE + out_all;
	...
	*((__global ulong *)out)       = *((__global ulong *)out)       ^ *((__global ulong *)iv);
	*(((__global ulong *)out) + 1) = *(((__global ulong *)out) + 1) ^ *(((__global ulong *)iv) + 1);
